...
class Rel501_Prev_Receb Extends  TPage
{
	...
	public function __construct()
	{
		...
		$this->form->addAction('Gerar Relatório2' , new TAction(array($this, 'onSelTeste2')), 'fa:print fas blue');//ico_pirnt.png **onDatagrid
	...
	public function onSel501($param)
	{
		try
        {
			...
			*/
            ...
            $designer->fromXml('app/reports/rel501.pdf.xml');
			
			$designer->replace('{DT_INI}', $data->DT_INI);
			$designer->replace('{NOME}', $row['NOME']);
			$designer->generate();
			
			//ÃNCORAS
			//foreach($result as $results )
			//{	
				/*$designer->gotoAnchorXY('CONTRATO_OLD');
			    $designer->setFont('Arial', '' ,10 );
			    $designer->Write(20, $row['CONTRATO_OLD']);*/
			    //$designer->Write(20, $results['CONTRATO_OLD'] );
			//}
			...
			$designer->gotoAnchorXY('DT_FIM');
			$designer->setFont('Arial', '' ,8);
			$designer->Write(20, $data->DT_FIM);
			
			$designer->gotoAnchorXY('ENT_COL');
			$designer->setFont('Arial', '' ,8 );
			$designer->Write(20,$data->ENTIDADES);
			//$designer->Write(20,$row['ENT_GAR']);
