...
class Rel501_Prev_Receb Extends  TPage
{
	...
	public function __construct()
	{
		...
		$btn = $this->form->addAction('PDF 502' , new TAction(array($this, 'onSel502')), 'fa:file fas blue ');
		...
		$this->form->addAction('Fechar' , new TAction(array($this, 'onSel501B')), 'fa: fa-power-off red');
	...
	{
		$relatorio = TSession::getValue('TS_cliente');
	...
	public function onSel501B()
	{
		try
		{
			$rel_501 = TSession::getValue('TS_cliente');
			
			$designer = new TReportHeaderFooter2;// classe filha da TPDFDesigner 
			...
			foreach( $rel_501 as $rels_501 )
			{
				...
				$designer->SetMargins(5,5,5);
				$designer->Cell( 100, 17, $rels_501['CONTRATO_OLD'], 0, 0, 'C', $fill);
				$designer->Cell(160, 17, utf8_decode($rels_501['NOME']), 0, 0, 'L', $fill);
				
				//$pdf->Cell(0,10, $dado['NOME'] ,0,1);
			...
			//$pdf = new TReportHeaderFooter2();
			// $pdf = new TReportHeaderFooter2();
			// $pdf->AliasNbPages();
			// $pdf->AddPage();
			// $pdf->SetFont('Times','',12);
			
			// $pdf->fromXml('app/reports/rel501_3.pdf.xml');
	...
	public function onSel502($param)
	{
		try
        {
			...
			*/
			...
			$designer = new TReportHeaderFooter2; 
			$designer->fromXml('app/reports/rel501_3.pdf.xml');
			$designer->generate();//tras todos
	...
	public function onSel501($param)
	{
		try
        {
			...
			*/
			...
			//$designer = new TPDFDesigner;
			$designer = new TReportHeaderFooter2;// classe filha da TPDFDesigner 
			$designer->fromXml('app/reports/Rel501A.pdf.xml');
			$designer->replace('{DT_INI}', $data->DT_INI );
			$designer->replace('{DT_FIM}', $data->DT_FIM );
			$designer->generate();//tras todos
			...
			{	
				$designer->gotoAnchorX('items');
				$designer->SetMargins(0,0,0);
				$designer->Cell( 100, 20, $rels_501['CONTRATO_OLD'], 0, 0, 'L', $fill);//x y 
				$designer->Cell(190, 20, utf8_decode($rels_501['NOME']), 0, 0, 'L', $fill);
				$designer->Cell(160, 20, utf8_decode($rels_501['CLI_FOR']), 0, 0, 'L', $fill);
				$designer->Cell(115, 20, utf8_decode($rels_501['ENTIDADE_COLETIVA']), 0, 0, 'L', $fill);
			...
			$designer->save('app/output/Rel501A.pdf');
			
			parent::openFile('app/output/Rel501A.pdf');
		   ...
		   //converter JSON
			$cli_json = TSession::getValue('TS_cliente');
					   
			$teste_jason = json_encode($cli_json);
					   
			var_dump($teste_jason);
					   
			TSession::setValue('TS_json', $teste_jason);		   
