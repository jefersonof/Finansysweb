...
class SystemUserList extends TStandardList
{
    ...
    public function __construct()
    {
		...
		// create EDIT2 action
        $action_edit2 = new TDataGridAction(array('FormUserSistem', 'onEdit'));
        $action_edit2->setButtonClass('btn btn-default');
        $action_edit2->setLabel('Edit 2');
        $action_edit2->setImage('far:edit blue');
        $action_edit2->setField('id');
        $this->datagrid->addAction($action_edit2);
    ...
    public function onClone($param)
    {
        try
        {
			...
			TTransaction::open('permission');
			//CLONA AS PERMISSÕES ESPECIAIS
			//pega o id criado no método onClone() pg 'SystemUser'
			$id_user = TSession::getValue('TS_idUser'); 
			
			$funcao = new funcao;
			$programas = $funcao->buscaUserProgram($param['id']);
			//var_dump($programas);
			foreach($programas as $programa)
			{	//var_dump($programa['system_program_id'] );
				//var_dump($programa['id'] . $programa['alteracao']  );
				$programa_user = new SystemUserGroupProgram;
				$programa_user->system_user_id    = $id_user;
				$programa_user->system_program_id = $programa['system_program_id'];
				$programa_user->acesso            = $programa['acesso'];
				$programa_user->insercao          = $programa['insercao'];
				$programa_user->alteracao         = $programa['alteracao'];
				$programa_user->delecao           = $programa['delecao'];
