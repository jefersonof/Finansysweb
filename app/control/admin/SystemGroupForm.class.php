...
class SystemGroupForm extends TPage
{
    ...
    function __construct()
    {
		...
		//permissão
        $checkpermissao  = new TCheckGroup('checkpermissao');
		...
		//permissão
		$options = ['insercao'=>'Inserção', 'alteracao' => 'Alteração', 'delecao' => 'Deleção'];
		
		$checkpermissao->setLayout('horizontal');
		$checkpermissao->addItems($options);
		...
		//permissão
		$this->form->addFields( [new TFormSeparator(('Controle de Atividades'))]);
		$this->form->addFields( [ new TLabel('Permissão') ],  [ $checkpermissao ]);
    ...
    public function onSave($param)
    {
        try
        {
			...
			//permissão
			// if ternário
			$array_opcs = $data->checkpermissao;//vem do form como array
			if(in_array('insercao', $array_opcs))//se exite a palavra 'insercao' n array 'array_opcs'
			{
				$object->insercao = 1;
			}
			else
			...
			if(in_array('delecao', $array_opcs))
			{
				$object->delecao = 1;
			}
			else
			...
			if(in_array('alteracao', $array_opcs))
			{
				$object->alteracao = 1;
			}
			else
    ...
    function onEdit($param)
    {
        try
        {
            if (isset($param['key']))
            {
				...
				//se a 'insercao/delecao' == 1 n DB trás o TCheckGroup marcado 				
				$insercao  = $object->insercao  == 1 ? 'insercao'  : 'A';		
				$delecao   = $object->delecao   == 1 ? 'delecao'   : 'A';		
				$alteracao = $object->alteracao == 1 ? 'alteracao' : 'A';		
				
				//monta o array
				$data_array = ['insercao'=>$insercao, 'alteracao' =>$alteracao, 'delecao' =>$delecao];
				
				//manda os dados para o 'TCheckGroup'
				$object->checkpermissao = $data_array;
