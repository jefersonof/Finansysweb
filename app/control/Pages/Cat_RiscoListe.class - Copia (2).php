...
{	
	...
	public function __construct()
	{
		...
		try
		{
			...
			$testes_permissao = TSession::getValue('usergroupids');
			//$testes_permissao = ['0' => 5, '1' => 4, '2' => 3, '3' => 2];;
			
			//var_dump($testes_permissao);
			
			$gp_permissao['acesso']     = 1;
			$gp_permissao['insercao']   = 0;
			$gp_permissao['alteracao']  = 0;
			$permissao_teste['delecao'] = 0;
			foreach($testes_permissao as $teste_permissao)
			{
				$grupo = new SystemGroup($teste_permissao);
				
				if($grupo->insercao == 1 )
				{
					$gp_permissao['insercao']  = $grupo->insercao; 
				}	
				
				// $gp_permissao['insercao']  = $grupo->insercao; 
				// $gp_permissao['alteracao'] = $grupo->alteracao; 
				// $gp_permissao['delecao']   = $grupo->delecao; 
				
				// if($gp_permissao['insercao'] == 1 )
				// {
					// $gp_permissao['insercao'] = 1;
				// }
				
				// if($gp_permissao['alteracao'] == 1 )
				// {
					// $gp_permissao['alteracao'] = $grupo->alteracao;
				// }
				
				// if($gp_permissao['delecao'] == 1 )
				// {
					// $gp_permissao['delecao'] = $grupo->delecao;
				// }
				//var_dump($teste_permissao);	
			}
			
			var_dump($gp_permissao['insercao']);
